      ...
      <div class="row align-items-center">
        <div class="col-md-12">
          <div class="title mb-30">
            <h2>ACTUALIZAR PRODUCTO / SERVICIO: <i><?= $consulta['DescripcionInv']; ?></i></h2>
          </div>
        </div>
      </div>
    </div>
    <div class="col-lg-12">
      <div class="card-style settings-card-2 mb-30">
        <form id="formulario" autocomplete="off" class="row">
            ...
            <div class="mb-3">
              ...
              <select class="form-select form-select-lg text-bold" name="IDTipoInv" required>
                ...
                <option value="<?= encriptar($row['IDTipoInv']); ?>"
                  <?= ($consulta['IDTipoInv'] == $row['IDTipoInv'] ? 'selected' : '') ?>>
          ...
          <div class="col-md-6">
            <?= renderInput('CODIGO DEL PRODUCTO / SERVICIO:', 'codigoInv', 'text', $consulta['CodigoInv'], true); ?>
          </div>
          <div class="col-md-6">
            <?= renderInput('DSCRIPCION DEL PRODUCTO / SERVICIO:', 'descripcionInv', 'text', $consulta['DescripcionInv'], true); ?>
          </div>
          <div class="col-md-6">
            <?= renderInput('PRECIO DEL PRODUCTO / SERVICIO:', 'precioInv', 'number', $consulta['PrecioInv'], true, 3, 'step="0.01"'); ?>
          </div>
          <div class="col-md-12">
...
formulario.addEventListener('submit', (e) => {
  ...
  Swal.fire({
    title: '¿ESTA SEGURO?',
    text: 'EL PRODUCTO / SERVICIO SERA ACTUALIZADO',
    icon: 'warning',
    ...
    if (result.isConfirmed) {
      ajaxFormularioPOST('modulos/inventario/inventarioActualizar.php?id=<?= $_GET['id'] ?>')
    }
  })
})
</script>